...
class SolutionOCR:
    ...
    def OCR(self, imageFile):
        ...
        result = outputSortingOCR(pytesseract.image_to_data(imageForOCR, lang=self.language, output_type=Output.DICT),self.minConfPts)
        results["results"].append(result)
        y = datetime.datetime.now() - x
        print(y)
        result = outputSortingOCR(pytesseract.image_to_data(gaussImage, lang=self.language, output_type=Output.DICT),self.minConfPts)
        results["results"].append(result)
        y = datetime.datetime.now() - x
        print(y)
        result = outputSortingOCR(pytesseract.image_to_data(blurImage, lang=self.language, output_type=Output.DICT),self.minConfPts)
        results["results"].append(result)
        y = datetime.datetime.now() - x
        print(y)
        output = comperOCR(results)
        y = datetime.datetime.now() - x
        print(y)
        output = multiplicityFilter(output)
        y = datetime.datetime.now() - x
        print(y)
        return output
